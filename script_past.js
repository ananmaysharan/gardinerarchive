...
map.on("load", async () => {
    ...
    const toggleabletagIds = ['all', 'undergardiner', 'transit', 'activism', 'people', 'aerial', 'construction', 'traffic'];
    ...
    for (const id of toggleabletagIds) {
        ...
        link.onclick = function (e) {
            ...
            if (clickedTag == 'all') {
                const allTags = ['undergardiner', 'transit', 'activism', 'people', 'aerial', 'construction', 'traffic']
                map.setFilter("photos", getTagsFilter(allTags)); //yeepee!
            } else {
            const allowedTags = []
